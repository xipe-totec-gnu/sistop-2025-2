...
int callSanta(void){
    sem_getvalue(&santaClaus, &disp);
    if( disp == 0){
        printf("Santa está ocupado");
        sem_post(&santaClaus);
    }
    else {
        sem_wait(&santaClaus);
...
void *elfo(void *arg) {
    ...
    while (1) {
        ...
        // Los renos regresan de sus vacaciones y esperan en la habitación de Santa
        sem_wait(&mutElf);
        printf("El elfo requiere ayuda.....\n");
        elfosEsperando++;
        printf("El número de elfos esperando afuera de la habitación de Santa son: %d\n", elfosEsperando);
